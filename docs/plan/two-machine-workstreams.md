...
6. **唯一交叉点**：B 线提供 `Resolver(actor, …)` 新签名（B-04）；A 线只调用、不改实现。
   ...
   - **依赖**：工具/模型授权行的 `subject` 维度由 B-03 提供（`workbuddy_tenant_{tool,model}_grants.subject_key`：`tenant` / `department:<id>` / `member:<user_id>`）。
...
| 4 | A-08、A-09、A-11、**A-10（超时与升级）**、**A-18（会话续期）** | B-10、B-12 | ✅ |
| 5 | A-12、A-13、A-14、A-15 | B-13、B-14、B-15 | ✅ |
| 6 | A-16、A-17、A-22、A-23、A-24 | B-11、B-16（待政策） | ✅ |

> 更正（集成者）：原批次表漏排了 `A-10`（超时与升级 SLA）与 `A-18`（会话续期 refresh token）两项，两者都补入批次 4——A-10 与 A-08 的 `waiting_input`/`waiting_approval` 是同一套等待机件，A-18 与工作流无关、体量小，放在同一批次不挤占关键路径。
