-- Schema v56: local ONNX embedding models as platform model revisions — SQLite
-- fail-closed marker.
--
-- The WorkBuddy platform catalog is PostgreSQL only, so the ONNX declaration
-- columns (and the check that keeps an ``onnx`` revision embeddable) are applied
-- on PostgreSQL alone. This file creates nothing on SQLite on purpose.
--
...
-- of serving a model catalog it cannot isolate.
--
