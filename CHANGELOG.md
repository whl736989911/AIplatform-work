...
- WorkBuddy **对话触发页**（A-17 前端）：WorkBuddy 控制台新增「对话触发」页（路由与侧栏同步）。页面只做两件事：**发一条消息**，以及**看这个会话触发过什么**——每条投递的状态、它启动的那次运行（可直接跳到运行页），以及**没跑起来的原因**（如 `DISPATCHER_UNAVAILABLE`）。后半件才是重点：一个「效果无人可查」的对话流程，不值得被信任；所以页面读的是投递账本，而不是只回一句「已发送」。**消息 id 在客户端生成并随消息发送**——它就是去重键，于是重发（网络抖动、手快点了两次）不会跑第二次工作流；空态如实说明「还没有触发过任何运行」。页面 2 例测试（消息带 id 发出且路径正确、投递与拒绝原因如实呈现）；路由/导航/双语文案（parity 通过）齐备；tsc 与 eslint 干净。

- WorkBuddy **Chatflow：消息可触发流程**（A-17 后端切片）：新增 `POST /conversations/{conversation_id}/messages`（128 条清单）。一个工作流想「回答某个会话」，就注册一条 **event 触发器**（`event_type = chat.message`）并在 `event_filter` 里写明它负责哪个会话——于是「这条消息该由谁处理」由**与 webhook 入站同一条匹配逻辑**回答（`event_matches_filter`），而不是另写一套判断；没有绑定的会话里发消息**照样受理**（没人监听是正常会话，不是错误）。**去重键是消息 id**（`chat:<message_id>`）：客户端重发或传输重复投递都不会跑第二次工作流——复用了投递账本本来就有的拒绝能力；**没有派发器就拒收并记录原因**（沿用 webhook 路径的 fail-closed），而不是把消息收下却无人应答。新增 `resolve_event_registrations`（本租户、按事件名、仅启用且未撤销）与 `CHAT_MESSAGE_EVENT` 常量。单测 4 例覆盖验收（消息命中绑定即触发且投递归属该执行、别的会话不触发、重发只跑一次、无派发器 fail-closed 并记录拒绝）；知识服务整文件 45 例全过；静态门禁全绿。**未包含**：会话页（前端）——本轮只做后端切片，界面留待下一步。

- WorkBuddy **定义画布（React Flow）与 JSON 双向**（A-16）：定义编辑器新增「JSON / 画布」切换，画布把同一份 definition 画成图，并只做**一种结构编辑：连线**。三个刻意的边界：**位置属视图、不是工作流**——schema 里没有坐标（一个盒子摆在谁屏幕的哪里，不是这个工作流做了什么），所以布局按工作流存在本地（前缀 `workbuddy-canvas-layout:`）并明确不属于定义，拖动因此**不会**改动 JSON；**新建节点归向导**（节点需要类型与 schema 的必填字段，那是元数据驱动表单的活），画布编辑的是「谁喂给谁」；**JSON 恒为事实来源**——画布只在文本可解析时才提供（正在改 JSON 时显示一张过期的图比不给更糟），连线变更立刻写回 JSON 文本交既有路由。转换是**纯函数**：`toCanvas`/`withCanvasEdges`/`sameConnections` 与确定性初始布局（按依赖深度分列，同一份定义每次长得一样；遇环不死循环——环是编译器的活）。展示与回写遵守**同一条规则**：指不存在的步骤的边两边都不保留，避免「画得出来但存不下去」。一处真实修复：`DefinitionCanvas.tsx` 与 `definitionCanvas.ts` 仅大小写不同，在大小写不敏感的文件系统上被解析到后者（组件 import 拿到 undefined），已重命名为 `definitionGraph.ts` 以消除歧义。纯逻辑 7 例 + 渲染 1 例；工作流目录与 parity 共 35 例全过；`tsc -b`、eslint 干净。**未包含**：拖拽连线的手势本身由 React Flow 提供、未做自动化手势测试（本机无真实服务人工冒烟）。

- WorkBuddy **运行指标面板**（A-24 前端）：工作台新增「运行指标」块，展示与提案门禁**同一套口径**的六个数字（已结算运行、成功率、P95 活跃时长、平均 token、平均等待、安全违规）以及**按工作流的分解**。块**明说自己在展示哪个作用域**（「仅你自己发起的运行」/「全租户」），因为把租户数字悄悄当成人个数字（或反之）比不展示更糟——作用域由服务端决定，前端不猜。数字全部来自服务端计算，前端不做二次口径；「没有已结算的运行」按**空态**处理而不是显示 0（0 读起来像「全都失败了」，那是另一回事）。双语键齐备（parity 通过）；块 3 例测试（作用域标签跟随服务端、数字与工作流名如实呈现、空态而非零值），首页降级用例同步由 3 块改为 4 块（并把块清单写进注释，今后加块要**有意**更新该数字）。

- WorkBuddy **运行指标口径与暴露**（A-24）：新增 `GET /execution-metrics`（127 条清单），**员工看自己发起的运行、管理员看整个租户**——两者算的是**同一套口径**，区别只在取哪些行。口径直接复用提案门禁那六个数字（结算数、成功率、p95 活跃时长、平均 token、安全违规、平均等待），因为「报表引用的数字」与「门禁据以判定的数字」若出自两套算法，迟早会讲出两个故事：新查询 `execution_metrics` 与 `canary_metrics` 同形同性——同一状态词汇（迁移 023 的 `success`/`canceled`）、同为整秒窗口且含结束秒、同样「只算已结算」（排队中或停放中的运行没有结果，计入只会让百分比与延迟都变好看）。聚合复用 `phase_metrics`，并把提案里那份指标载荷函数由私有改为公开（`metrics_payload`，3 处调用点同步）。作用域由**查询**施加（成员传 `created_by_user_id`），不靠调用方记得收窄——忘记收窄也宽不出去；返回体带 `scope`、窗口、总计与**按工作流的分解**（按样本数降序）。端到端 1 例覆盖验收：成员只见自己那 1 条、管理员见 2 条且成功率一致、尚未结算的运行不计入。

- WorkBuddy **outbox 传输实体与死信可查**（A-23）：派发器（认领→发布→有界退避→达上限转死信）此前已完整，但**没有任何实现**它发布的端口，于是部署到这个仓库的事件只会永远停在 `pending`。现在补上契约指定的那个实体：`RedisOutboxPublisher` 用 `LPUSH`（**不是** `PUBLISH`）把事件投到「每主题一条队列」——承诺的是已提交事件的**至少一次**投递，列表会把事件留到有消费者取走，而 pub/sub 会把当时没人订阅的东西丢掉；事件 **id 随正文一起走**，消费方据此对 PostgreSQL（唯一事实来源）去重，而不是相信通道的投递次数。列表**刻意不裁剪**（为控内存丢掉最旧的条目，正好破坏这个类存在的理由 ✓ 保留策略属于消费方 ✓）。`resolve_redis_publisher()` 沿用仓库既有约定（`REDIS_URL` + 惰性 `import redis` + 3 秒超时 ✓）：未配置时返回 `None` 而**不是**一个假装能投递的对象，于是派发器继续 fail-closed（事件留在 pending 且可见 ✓），而不是启动即失败。传输拒绝会带上稳定 `code`（`OUTBOX_TRANSPORT_FAILED`）抛给派发器——那正是死信里记录的错误文本。**死信可查**：`GET /outbox/dead-letters`（仅租户 admin，126 条清单）返回本租户投递失败的事件与各自最后一次传输错误；读取**显式按租户过滤**（它在平台上下文里读，不依赖 RLS——漏掉过滤就会看到所有租户的失败），且**只读不可重放**：失败八次之后是否还值得投递，是人的决定，不是重试循环能替人做的。单元 3 例（投递到正确主题队列且带 id ✓ 传输拒绝带 code ✓ 未配置即无 publisher ✓）+ 端到端 2 例（死信带错误可读、待投递的不是死信、跨租户为空、成员 403）。
